...
function huntEmail {
    ...
    process {
        ...
        if ($UserPrincipalName) {
            ...
            Write-Host "[SEARCH] Enumerating all licensed mailboxes..." -ForegroundColor Cyan
            try {
                $Mailboxes = @()
                $NextUrl = "https://graph.microsoft.com/v1.0/users?`$filter=assignedLicenses/`$count ne 0 and accountEnabled eq true&`$count=true&`$select=userPrincipalName,displayName,mail&`$top=999"
                $Headers = @{ "ConsistencyLevel" = "eventual" }
                do {
                    $UserResp = Invoke-MgGraphRequest -Method GET -Uri $NextUrl -Headers $Headers -ErrorAction Stop
                    $Users = $UserResp.value | Where-Object { $_.mail }
                    $Mailboxes += $Users.userPrincipalName
                    $NextUrl = $UserResp.'@odata.nextLink'
                } while ($NextUrl)
                Write-Host "[SEARCH] Found $($Mailboxes.Count) mailboxes to search" -ForegroundColor Cyan
            }
            catch {
